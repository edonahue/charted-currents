...
During Packet 1, `docs/PACKET1_DIRECTION.md` is required context. It settles the development anchors, quiet basemap, responsive inspector, camera, timeline, typography, visual-reference board, and maker-identity choices; do not reopen them through a new preference survey.
...
- Preserve `src/lib/domain/types.ts`, `src/lib/paths.ts`, `src/lib/data/loadPublished.ts`, `src/lib/map/config.ts`, `src/lib/map/developmentAnchors.ts`, and `src/lib/time/config.ts` as canonical starting points rather than duplicating their vocabularies/configuration.
