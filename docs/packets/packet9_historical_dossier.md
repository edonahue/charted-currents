...
* **Acquired Component Units**:
  * `HCA 32-11.380`: *Schip Nostra Seniora Concepcion y St Joseph van Cadiz onder leiding van Antonio de Witte; 1666* (Persistent Handle: `hdl:10648/aa19a57e-e115-094f-e053-09f0900ae341`, 5 physical digitized scans).
  * `HCA 32-11.391`: *Schip Nostra Seniora Concepcion y St Joseph onder leiding van Antonio de Witte; 1666* (Persistent Handle: `hdl:10648/aa19a57e-e120-094f-e053-09f0900ae341`, 7 physical digitized scans).
...
In accordance with `docs/HISTORICAL_ASSERTION_POLICY.md` and `docs/HISTORICAL_REVIEW_POLICY.md`, all 10 assertions added in Packet 9 are classified by epistemic risk and review burden:

### Class A — Direct Archival Transcription (9 assertions)
...
* `ast_na_380_origin_place_mapping`: Deterministic catalogue place mapping derived directly from Class A assertion `ast_na_380_origin_descriptor` (`"van Cadiz"`). Maps to `place_id: "place_cadiz"` with `derived_value: "Cádiz"`. Conflation of raw values with derived place mappings is strictly prevented. In accordance with P9-R6, the modern Dutch toponym attestation was removed from `place_cadiz` so that only seventeenth-century historical attestations remain on the canonical entity.
...
* **Vessel Resolution**: Two separate archival occurrences are modeled:
  * `occ_ship_nostra_seniora_380`: Raw name `"Nostra Seniora Concepcion y St Joseph"`, recorded origin Cádiz (`"van Cadiz"`), master Antonio de Witte, year 1666 (from `sr_na_hca32_11_380`).
  ...
  * **Scholarly Resolution Rationale & Counter-Hypothesis Analysis**:
    1. *Identity Hypothesis*: The identical Spanish religious invocation (*Nostra Seniora Concepcion y St Joseph*), exact master name (*Antonio de Witte*), identical year (*1666*), and provenance in the **same HCA32-11 inventory sequence** (Dutch finding aid 2.22.24 derived from TNA Kew HCA 32/11) indicate that items .380 and .391 represent papers from the same captured vessel or prize cause.
    2. *Counter-Hypothesis*: The two inventory numbers could represent distinct packets of intercepted letters forwarded under different legal proceedings, or two related vessels in a convoy bearing the same patronal name. Neither inventory entry contains an explicit cross-reference affirming they are the exact same physical hull.
...
* **Person Resolution**: Two separate archival person occurrences are modeled:
  * `occ_person_antonio_de_witte_380`: Raw name `"Antonio de Witte"`, role `"master"`, vessel `"Nostra Seniora Concepcion y St Joseph"`, year 1666.
...
| **2. Evidence precedes entities** | **PASS** | Validated unbroken chain: `Source (src_na_2_22_24_sailing_letters) → Source Records (sr_na_hca32_11_380, sr_na_hca32_11_391) → Assertions (10) → Separate Occurrences (2 ship occs, 2 person occs) → Class D Edges (4) → Canonical Entities (2)`. |
| **3. Raw values immutable** | **PASS** | Verified institutional spelling `"Nostra Seniora Concepcion y St Joseph"` preserved across .380 and .391. Dutch prepositional phrase `"van Cadiz"` preserved as immutable raw value. |
...
| **6. Honest inspection state** | **PASS** | Source records declare `inspection_state: metadata_only`. Upstream custody at TNA Kew is explicitly documented and registered under `src_tna_hca_32`. |
...
| **23. Preserve contradictory evidence** | **PASS** | Orthography verified against live institutional EAD and official inventory downloads; any future contradictory source transcriptions will be preserved occurrence-by-occurrence. |
