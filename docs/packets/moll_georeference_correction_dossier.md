...
*Note on population difference*: Candidate C covers 13 points spanning the entire map plate, including peripheral anchors (Charles Town, SC, St. Augustine, FL, and Bridgetown, Barbados). Candidate F1 is a full-chart affine fit using strictly the 10 inner-basin controls without peripheral constraints. Both candidates use the same six-parameter affine transformation. Candidate F1 uses three fewer GCP observations / constraints (10 versus 13). The headline LOOCV RMSEs (107.68 km vs 101.16 km) evaluate different geographic extents and control populations.
...
| **LOOCV Median Error** | **83.11 km** | 97.94 km | **-14.83 km** (C is lower) |
